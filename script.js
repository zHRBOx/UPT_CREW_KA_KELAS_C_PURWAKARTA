...
// PERBAIKAN: Membuat struktur data master yang tidak akan pernah berubah.
...
// PERBAIKAN: `appData` sekarang diinisialisasi sebagai deep copy, melindunginya dari modifikasi.
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // PERBAIKAN: Logika pemuatan data yang baru dan aman
    const cloudData = await downloadDataSecurely(); 
    if (cloudData) {
        appData = deepMerge(JSON.parse(JSON.stringify(DEFAULT_APP_DATA)), cloudData);
    ...
    for (const key of Object.keys(sections)) {
        ...
        sectionEl.id = key;
        sectionEl.className = 'content-section hidden';
...
function switchTab(group, tabId) {
    ...
    if(activeContent) {
        ...
        activeContent.classList.add('active'); // PERBAIKAN: tambahkan class active untuk konsistensi
    } else {
...
function loadDataFromStorage() {
    ...
    if (savedDataString) {
        try {
            ...
            appData = deepMerge(JSON.parse(JSON.stringify(DEFAULT_APP_DATA)), savedData);
            ...
            appData = JSON.parse(JSON.stringify(DEFAULT_APP_DATA)); // Fallback to default
        }
    }
}
...
function manageScheduleRotation() {
    ...
    if (dayDifference >= 1) {
        if (dayDifference === 1) {
            ...
            appData.jadwalHariIni = JSON.parse(JSON.stringify(appData.jadwalBesok ?? DEFAULT_APP_DATA.jadwalBesok));
...
function addTableRow(dataKey, subKey = null, thirdKey = null) {
    ...
    if (Array.isArray(targetArray) && targetArray.length > 0) {
        ...
        // This switch provides the structure for a new row if the array is empty
        switch (lookupKey) {
...
function render_pegawai() {
    ...
    if (!contentEl.innerHTML.includes('pegawaiTable')) {
        contentEl.innerHTML = `<div class="bg-white p-6 rounded-lg shadow"><h2 class="text-2xl font-bold text-[#0D2B4F] mb-4">Daftar Nominatif Pegawai</h2><input type="text" id="searchInput" onkeyup="searchTable('pegawaiTable')" placeholder="Cari..." class="w-full p-2 border border-gray-300 rounded-lg mb-4"><div class="overflow-x-auto" style="max-height: 70vh;"><table id="pegawaiTable" class="w-full text-sm text-center whitespace-nowrap"></table></div><div id="pegawaiTable-controls" class="mt-4"></div></div>`;
    }
...
// ... ALL OTHER RENDER AND SAVE FUNCTIONS ...

// Helper to calculate employee composition for the chart
...
// Helper to create the dashboard chart
